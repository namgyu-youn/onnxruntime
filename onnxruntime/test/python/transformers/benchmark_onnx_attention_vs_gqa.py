...
Arms, all decode-shaped (S_q = 1, causal, no RoPE, no attn_mask, no softcap):
  ...
  attn_scatter  ONNX Attention opset-24 external cache: in-place TensorScatter
                ...
                them adds a full-cache copy). Pinned to the Flash tier.
  attn_scatter_cudnn
                attn_scatter with the cuDNN SDPA decode tier (PR #29715)
                enabled; aborts unless the build resolves it.
...
Measurement notes:
  ...
  - Before timing, arms in EXPECTED_KERNEL assert their resolved backend
    (SdpaKernel debug print) and every session is checked for nodes placed
    off the CUDA EP; verify the other arms from an nsys capture (--profile).
...
# Both ops fall back silently when a pinned backend is ineligible, so build_arm
...
def build_arm(arm, args, past_seq_len, torch_dtype, canonical=None):
    ...
    mismatch (arms in EXPECTED_KERNEL) or off-CUDA node placement."""
    if arm in EXPECTED_KERNEL:
        ...
        del session
        if resolved != EXPECTED_KERNEL[arm]:
            hint = ""
            if resolved is None and arm.startswith("attn_"):
                hint = " (no SdpaKernel debug print: this arm needs a build with PR #29715)"
            sys.exit(f"{arm}: resolved SdpaKernel={resolved}, expected {EXPECTED_KERNEL[arm]}{hint}")
