...
// Block-wise (GroupSize > 0) MoE GEMV kernel. Mirrors moe_gemv_kernel but reloads
// scales each CTA K-tile iteration from a [E, K/GroupSize, N] scale buffer.
// GroupSize must satisfy: (CtaK / kInterleave) % GroupSize == 0.
template <typename Details, int CtaN, int Threads, bool EnableBias, int GroupSize,
          typename TypeA = typename Details::TypeDetailsA::Type>
__global__ void moe_gemv_block_wise_kernel(TypeA* act, uint8_t* weight, TypeA* scales, TypeA* bias, TypeA* out,
                                           int64_t const* expert_first_token_offset, int const* permuted_row_to_expert,
                                           int num_experts,
                                           int64_t weight_expert_stride, int n, int k) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 750))
  ...
  static constexpr int CtaK = StepK * Threads;
  static_assert(CtaN % 2 == 0);
  static_assert(GroupSize > 0 && (CtaK / Details::kInterleave) % GroupSize == 0);
...
#pragma unroll 1
  ...
  weight += expert * weight_expert_stride;
  // scales layout: [E, K/GroupSize, N] (PrePack-transposed)
  scales += static_cast<int64_t>(expert) * (k / GroupSize) * n;
  if constexpr (EnableBias) {
    bias += static_cast<int64_t>(expert) * n;
  }
  ...
  GMemIterator<Mandatory, AccessTypeW, CtaN, Details::kAccessNumW, uint8_t> weight_iterator(
      weight, (interleaved_offset_n * interleaved_k + tid * StepK) / Details::kElemsPerByteW,
      CtaK / Details::kElemsPerByteW, interleaved_k / Details::kElemsPerByteW);
  // Block-wise: initial offset includes k-block index; step advances by one k-block per outer iter.
  static constexpr int kScaleStep = (CtaK / Details::kInterleave / GroupSize);
  GMemIterator<Mandatory, TypeA, CtaN, 1, TypeA> scales_iterator(
      scales, real_offset_k / GroupSize * n + real_offset_n, kScaleStep * n, Details::kInterleave);
...
template <typename Details, int CtaN, int Threads, bool EnableBias, int GroupSize,
          typename TypeA = typename Details::TypeDetailsA::Type>
__global__ void moe_gemv_interleaved_swiglu_block_wise_kernel(
    TypeA* act, uint8_t* weight, TypeA* scales, TypeA* bias, TypeA* out,
    int64_t const* expert_first_token_offset, int const* permuted_row_to_expert, int num_experts,
    int64_t weight_expert_stride, int inter_size, int k, cutlass_kernels::ActivationParams activation_params) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 750))
...
static void launch_moe_gemv_block_wise(TypeA* act, uint8_t* weight, TypeA* scales, TypeA* bias, TypeA* out,
                                       int64_t const* expert_first_token_offset, int const* permuted_row_to_expert,
                                       int num_experts, int64_t expanded_num_rows, int64_t n, int64_t k,
                                       cudaStream_t stream) {
  int64_t const weight_expert_stride = n * k / Details::kElemsPerByteW;
  ...
  dim3 block(Threads);
  if (bias != nullptr) {
    moe_gemv_block_wise_kernel<Details, CtaN, Threads, true, GroupSize><<<grid, block, 0, stream>>>(
        act, weight, scales, bias, out, expert_first_token_offset, permuted_row_to_expert, num_experts,
        weight_expert_stride, static_cast<int>(n), static_cast<int>(k));
  } else {
    moe_gemv_block_wise_kernel<Details, CtaN, Threads, false, GroupSize><<<grid, block, 0, stream>>>(
        act, weight, scales, bias, out, expert_first_token_offset, permuted_row_to_expert, num_experts,
        weight_expert_stride, static_cast<int>(n), static_cast<int>(k));
  }
}
...
template <typename Details, int CtaN, int Threads, int GroupSize, typename TypeA>
static void launch_moe_gemv_interleaved_swiglu_block_wise(
    TypeA* act, uint8_t* weight, TypeA* scales, TypeA* bias, TypeA* out,
    int64_t const* expert_first_token_offset, int const* permuted_row_to_expert, int num_experts,
    int64_t expanded_num_rows, int64_t inter_size, int64_t k,
    cutlass_kernels::ActivationParams activation_params, cudaStream_t stream) {
  ...
  dim3 block(Threads);
  if (bias != nullptr) {
    moe_gemv_interleaved_swiglu_block_wise_kernel<Details, CtaN, Threads, true, GroupSize><<<grid, block, 0, stream>>>(
        act, weight, scales, bias, out, expert_first_token_offset, permuted_row_to_expert, num_experts,
        weight_expert_stride, static_cast<int>(inter_size), static_cast<int>(k), activation_params);
  } else {
    moe_gemv_interleaved_swiglu_block_wise_kernel<Details, CtaN, Threads, false, GroupSize><<<grid, block, 0, stream>>>(
        act, weight, scales, bias, out, expert_first_token_offset, permuted_row_to_expert, num_experts,
        weight_expert_stride, static_cast<int>(inter_size), static_cast<int>(k), activation_params);
  }
}
...
bool is_moe_gemv_supported(int sm, int64_t expanded_num_rows, int64_t n, int64_t k, int group_size) {
...
template <typename T>
void launch_moe_gemv_int4_block_wise(T const* act, uint8_t const* weight, T const* scales, T const* bias, T* out,
                                     int64_t const* expert_first_token_offset, int const* permuted_row_to_expert,
                                     int num_experts,
                                     int64_t expanded_num_rows, int64_t n, int64_t k, int group_size,
                                     int sm, cudaStream_t stream) {
  ORT_UNUSED_PARAMETER(sm);
  ORT_ENFORCE(group_size == 128, "Only group_size=128 is currently supported for block-wise MoE GEMV");
  using Details = typename DetailsForT<T>::Details;
  using TypeA = typename DetailsForT<T>::TypeA;
  fiv::launch_moe_gemv_block_wise<Details, kCtaN, kThreads, 128, TypeA>(
      const_cast<TypeA*>(reinterpret_cast<TypeA const*>(act)),
      const_cast<uint8_t*>(weight),
      const_cast<TypeA*>(reinterpret_cast<TypeA const*>(scales)),
      const_cast<TypeA*>(reinterpret_cast<TypeA const*>(bias)),
...
void launch_moe_gemv_int4_block_wise_interleaved_swiglu(
  ...
  using Details = typename DetailsForT<T>::Details;
  using TypeA = typename DetailsForT<T>::TypeA;
  fiv::launch_moe_gemv_interleaved_swiglu_block_wise<Details, kCtaN, kThreads, 128, TypeA>(
      const_cast<TypeA*>(reinterpret_cast<TypeA const*>(act)),
      const_cast<uint8_t*>(weight),
      const_cast<TypeA*>(reinterpret_cast<TypeA const*>(scales)),
      const_cast<TypeA*>(reinterpret_cast<TypeA const*>(bias)),
...
template void launch_moe_gemv_int4_block_wise<half>(half const*, uint8_t const*, half const*, half const*, half*,
                                                    int64_t const*, int const*, int, int64_t, int64_t, int64_t,
                                                    int, int, cudaStream_t);
template void launch_moe_gemv_int4_block_wise_interleaved_swiglu<half>(
    half const*, uint8_t const*, half const*, half const*, half*, int64_t const*, int const*, int, int64_t,
    int64_t, int64_t, int, int, cutlass_kernels::ActivationParams, cudaStream_t);
