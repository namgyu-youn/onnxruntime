...
for dtype in float16 bfloat16; do
  ...
  # 4-D cache: the cuDNN tier's mixed-layout path; scatter arms only.
  (cd "$BENCH" && python benchmark_onnx_attention_vs_gqa.py --sanity --dtype "$dtype" \
      --cache-4d --arms attn_scatter attn_scatter_cudnn) \
    2>&1 | tee "results/sanity_4d_${dtype}.log"
done

echo
echo "Check all logs: every parity line PASS; 'use_xqa' printed for gqa_xqa only."
